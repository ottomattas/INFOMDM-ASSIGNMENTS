...
def scoreBayes(train_ngram,
    ...
    for smoothingfactor in tqdm(np.arange(sfactor_start, sfactor_end,
                                          sfactor_step),
                                desc="Testing different smoothing factors",
                                leave=False):
...
def scoreLogReg(train_ngram,
    ...
    for lambd in tqdm(np.arange(lambda_start, lambda_end, lambda_step),
                      desc="Testing different lambda values",
                      leave=False):
...
def scoreRandomForests(train_ngram,
                       train_classes,
                       test_ngrams,
                       test_classes,
                       ntrees_start=50,
    ...
    for ntrees in tqdm(np.arange(ntrees_start, ntrees_end, ntrees_step),
                       desc="Testing different number of trees",
                       leave=False):
        for featprop in tqdm(np.arange(nfeats_start, nfeats_end, nfeats_step),
...
def scoreDecisionTree(train_ngram,
                      train_classes,
                      test_ngrams,
                      test_classes,
                      alpha_range=(0.0, 10, 0.1),
                      maxdepth_range=(5, 50, 1),
                      minleaf_range=(2, 30, 2),
    ...
    for maxdepth in tqdm(range(maxdepth_start, maxdepth_end, maxdepth_step),
                         desc="Testing different tree depths",
                         leave=False):
        for alpha in tqdm(np.arange(alpha_start, alpha_end, alpha_step),
...
if __name__ == '__main__':
    files = [
        str(x) for x in Path('Resources' + os.path.sep +
                             'op_spam_v1.4').rglob('*.txt')
    ...
    important_features(trainvectorizer, bayes_unigram["classifier"])
    ...
    dtree_unigram = scoreDecisionTree(unigrams, trainclasses, testdocs,
                                      testclasses)
    important_features(trainvectorizer, dtree_unigram["classifier"])
    ...
    dtree_unibigram = scoreDecisionTree(unigrams, trainclasses, testdocs,
                                        testclasses)
    print("Best Decision Tree classifier for unigram and bigram features:\n" +
